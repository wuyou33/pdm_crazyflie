...
//	Xe_ob = Eigen::Matrix<double,NUM_STATE,1>::Zero();
	...
	A_model(6,3) = -6.5746;	A_model(6,6) = -2.9744;
	A_model(7,4) = -6.5746;	A_model(7,7) = -2.9744;
	A_model(8,5) = -5.6689;	A_model(8,8) = -2.6190;

	//B_model
	B_model(6,0) = 0.4865;
	B_model(7,1) = 0.4865;
	B_model(8,3) = 0.0006519;
...
//////////////////////////////////////////////////////////////////////////////
//誘導制御演算(MPC)
Eigen::MatrixXd CGdnCont::GdnCont(Eigen::Matrix<double,NUM_IMPUT*N,1> U,double pos[3],double vel[3],double lin[3])
{
...
////////同一次元オブザーバ(20160907)
	...
	if(true == ObserverFlag)
	{
		X_ob(0,0) = pos[0] - ref_X;
		X_ob(3,0) = pos[1] - ref_Y;
		X_ob(6,0) = pos[2] - ref_Z;
		ObserverFlag = false;
	}

	U_ob << X(0,0),			//U_ob = [x,vx,φ,y,vy,θ,z,vz,T]'
		X(3,0),
		U(0,0),
		X(1,0),
		X(4,0),
...
////////デバッグ用
	//std::cout << "U:" << (U.block(0,0,NUM_IMPUT,1)).transpose() << std::endl;
	//std::cout << "dU:" << (dU.block(0,0,NUM_IMPUT,1)).transpose()*dt << std::endl << std::endl;
////////
