...
CGdnCont::CGdnCont(void)
{
	...
	this->S(NUM_STATE,NUM_STATE);		this->S = Eigen::Matrixd6::zero();
	this->Q(NUM_STATE,NUM_STATE);		this->Q = Eigen::Matrixd6::zero();
	this->R(NUM_IMPUT,NUM_IMPUT);		this->R = Eigen::Matrixd4::zero();
...
Eigen::MatrixXd<double, const double&> CGdnCont::CalcF(Eigen::MatrixXd<double, const double&> ustar, Eigen::MatrixXd(<double, const double&> xinit)
{
	...
	for(i = 0; i < N; i++){
		...
		f(5,0) = -kz*xstar(5,i) + ustar(NUM_IMPUT*i+2,0) / m; // + m*g
//モデルを変更
